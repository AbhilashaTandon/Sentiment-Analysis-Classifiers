...
train_ratings, dev_ratings, train_reviews, dev_reviews = train_test_split(
	train_ratings+dev_ratings, train_reviews+dev_reviews, test_size=0.4, random_state=42
)

train_ratings = list(train_ratings)
train_reviews = list(train_reviews)
dev_ratings = list(dev_ratings)
dev_reviews = list(dev_reviews)

print(len(train_ratings), len(train_reviews), len(dev_ratings), len(dev_reviews))
...
def get_tf_idfs(reviews): #calculates tf-idf scores for each doc, returns those and a list of words in all docs
...
def main():
	train_tf_idfs = get_tf_idfs(train_reviews) #get training data tf-idfs
	
	dev_tf_idfs = get_tf_idfs(dev_reviews) #get development data tf-idfs
